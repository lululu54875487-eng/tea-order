...
管理者可以直接在網頁新增常用店家，不需要修改程式碼：

1. 選擇 `飲料` 或 `餐食`。
2. 輸入新店家名稱。
3. 在 Menu 區塊按 `＋` 新增品項與價格。
4. 菜單完成後，按「存成常用店家」。
5. 若已串接 Google Apps Script，常用店家會跟訂單一起存到 Google Sheets。

之後再輸入該店名，或點選店家快捷按鈕，就會自動帶出菜單。若要刪除自己新增的店家，選到該店家後按「刪除自訂店家」。
...
此專案已提供 `apps-script/Code.gs`。它會把整份訂單狀態存在 Google Sheets 的 `Orders` 工作表，並把訂單清單展開在第 4 列之後，方便人工查看。自訂常用店家也會輸出到 `Templates` 工作表。
